...
import BirthdayAwardHistory from "./BirthdayAwardHistory";
...
const TIP_BADGE_CONFIG: Record<string, { symbol: string; color: string; bg: string }> = {
  ...
  "Obsessed With the MMC":    { symbol: "⚔️", color: "#60a5fa", bg: "rgba(96,165,250,0.15)"  },
...
export default async function ProfilePage() {
  ...
  const { data } = await supabase
    ...
    .select(
      "username, pen_name, avatar_url, banner_url, bio, writes_genres, writer_level, publishing_goals, feedback_areas, feedback_preference, is_public, highlighted_manuscript_id, social_tiktok, social_facebook, social_instagram, social_x, social_snapchat, social_threads, social_lemon8, social_goodreads"
    )
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  // Fetch author's top-3 tip badges (publicly readable; RLS policy is USING(true))
  const { data: tipBadgesRaw } = await supabase
          ...
          <div className="absolute bottom-3 left-3 z-20 flex flex-wrap gap-2">
            ...
            {isAdult && (
              <>
                ...
                {p?.social_goodreads && (
                  <a href={`https://goodreads.com/${p.social_goodreads}`} target="_blank" rel="noopener noreferrer"
                    title={`Goodreads: ${p.social_goodreads}`}
                    className="flex h-8 w-8 items-center justify-center rounded-full backdrop-blur-sm transition hover:scale-110" style={{ background: "#59461b" }}>
                    <svg viewBox="0 0 448 512" className="h-4 w-4 fill-white" aria-hidden="true"><path d="M299.9 191.2c5.1 37.3-4.7 79-35.9 100.7-22.3 15.5-52.8 14.1-70.8 5.7-37.1-17.3-49.5-58.6-46.8-97.2 4.3-60.9 40.9-87.9 75.3-87.5 46.9-.2 71.8 31.8 78.2 78.3zM448 88v336c0 30.9-25.1 56-56 56H56c-30.9 0-56-25.1-56-56V88c0-30.9 25.1-56 56-56h336c30.9 0 56 25.1 56 56zM330 313.2s-.1-34-.1-217.3h-29v40.3c-.8.3-1.2-.5-1.6-1.2-9.6-20.7-35.9-46.3-76-46-51.9.4-87.2 31.2-100.6 77.8-4.3 14.9-5.8 30.1-5.5 45.6 1.7 77.9 45.1 117.8 112.4 115.2 28.9-1.1 54.5-17 69-45.2.5-1 1.1-1.9 1.7-2.9.2.1.4.1.6.2.3 3.8.2 30.7.1 34.5-.2 14.8-2 29.5-7.2 43.5-7.8 21-22.3 34.7-44.5 39.5-17.8 3.9-35.6 3.8-53.2-1.2-21.5-6.1-36.5-19-41.1-41.8-.3-1.6-1.3-1.3-2.3-1.3h-26.8c.8 10.6 3.2 20.3 8.5 29.2 24.2 40.5 82.7 48.5 128.2 37.4 49.9-12.3 67.3-54.9 67.4-106.3z"/></svg>
                  </a>
                )}
              </>
            )}
