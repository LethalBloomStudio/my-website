...
export default function ManuscriptDetailsPage() {
  ...
  async function load() {
    ...
    if (readerIds.length > 0) {
      const [{ data: profileRows }, conductRes, { data: badgeRows }] = await Promise.all([
        ...
        supabase.from("reader_badges").select("reader_id, active_badge").in("reader_id", readerIds),
      ]);
      ...
      setAcceptedReaders(
        readerIds.map((id) => ({
          ...
          active_badge: badgeMap.get(id) ?? null,
        })),
      );
    } else {
      setAcceptedReaders([]);
                  ...
                  {Array.from({ length: readerSlots }).map((_, i) => {
                    ...
                    return reader ? (
                      <div key={reader.user_id} className="flex shrink-0 flex-col items-center gap-1.5 group">
                        ...
                        {reader.active_badge && BADGE_CONFIG[reader.active_badge] && (
                          <span
                            title={reader.active_badge}
                            style={{ color: BADGE_CONFIG[reader.active_badge].color, background: BADGE_CONFIG[reader.active_badge].bg }}
                            className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold leading-none cursor-default"
                          >
                            {BADGE_CONFIG[reader.active_badge].symbol}
                          </span>
                        )}
                      </div>
                    ) : (
