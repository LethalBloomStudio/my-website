          ...
          {withUser && (hiddenFriends.some((f) => f.userId === withUser) || blockedUsers.some((f) => f.userId === withUser)) ? (
            <section className="flex h-[600px] flex-col rounded-xl border border-neutral-800 bg-[rgba(120,120,120,0.05)] p-4">
              <div className="mb-3 shrink-0 flex items-center gap-3">
                ...
                <button onClick={() => router.replace("/messages")} className="text-xs text-neutral-500 hover:text-white transition border border-neutral-700 rounded-lg px-2 py-0.5">Close</button>
              </div>
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((m) => (
                    ...
                    >
                      ...
                      >
                        <p>{m.body}</p>
                        <p className="mt-1 text-xs text-neutral-500">{new Date(m.created_at).toLocaleString()}</p>
                      </div>
              ...
              <p className="mt-3 shrink-0 text-xs text-neutral-600 italic">
                {blockedUsers.some((f) => f.userId === withUser)
            ...
            <section className="rounded-xl border border-[rgba(120,120,120,0.45)] bg-[rgba(120,120,120,0.18)] p-5">
              ...
              <div className="mt-5 rounded-lg border border-neutral-800 bg-neutral-900/40 p-3 text-sm space-y-1">
                ...
                {(status?.lifetime_suspension_count ?? 0) > 0 && (
                  <p className="text-neutral-500 text-xs">Total suspensions on record: {status?.lifetime_suspension_count}. Repeated violations may result in a permanent ban.</p>
                )}
              </div>
              ...
              {status?.parent_report_restricted && (
                <div className="mt-4 rounded-lg border border-red-700/50 bg-red-950/20 p-4">
                  ...
                  <p className="text-xs text-neutral-400 mb-3">Your account has been temporarily restricted from messaging and beta reading following a report from a parent account. An admin will review the report. If you believe this is in error, you may submit an appeal.</p>
                  {prAppealSubmitted ? (
            ...
            <section className="flex flex-col rounded-xl border border-[rgba(120,120,120,0.45)] bg-[rgba(120,120,120,0.18)] p-4">
              <div className="mb-3 shrink-0 flex items-center gap-3">
