...
export async function POST(req: Request) {
  ...
  // Check account is actually suspended or blacklisted (messaging)
  const { data: account } = await admin
    .from("accounts")
    .select("blacklisted, messaging_suspended_until, appeal_requested")
    .eq("user_id", userId)
    .maybeSingle();

  const isSuspended =
    account?.messaging_suspended_until &&
    new Date(account.messaging_suspended_until).getTime() > Date.now();

  if (!account?.blacklisted && !isSuspended) {
    return NextResponse.json({ error: "Your account is not currently suspended or blacklisted." }, { status: 400 });
  }
  ...
  if (adminIds.length > 0) {
    await admin.from("system_notifications").insert(
      adminIds.map((adminId) => ({
        ...
        title: "New Conduct Appeal",
        body: `${displayName} has submitted an appeal for their account suspension or blacklist. Appeal ID: ${(appeal as { id: string }).id}`,
        severity: "warning",
        dedupe_key: `appeal-${(appeal as { id: string }).id}-admin-${adminId}`,
      }))
    );
  }
